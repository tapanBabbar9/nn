#!/usr/bin/env python3
"""
Train a tiny LoRA (or QLoRA) adapter on movie review sentiment analysis.

Usage:
  python train.py                    # QLoRA on CUDA, LoRA fp16 on MPS/CPU
  python train.py --no-qlora         # Full-precision LoRA (needs more VRAM)
  python train.py --rank 16          # Higher rank = more adapter capacity
"""
...
from config import LoRAConfig, ModelConfig, QLoRAConfig, TrainConfig
from utils import (
    count_parameters,
...
def parse_args():
    p = argparse.ArgumentParser(description="LoRA / QLoRA fine-tuning")
...
def main():
    ...
    print("=" * 60)
    print("LoRA Fine-Tuning — Movie Reviews")
    print("=" * 60)
    ...
    # Save run metadata for compare.py / interviews
    meta = {
    ...
    # Save adapter weights only (~few MB vs ~6GB full model)
    model.save_pretrained(train_cfg.output_dir)
    tokenizer.save_pretrained(train_cfg.output_dir)
    print(f"\nAdapter saved to {train_cfg.output_dir}")
    print("Run: python compare.py  to see base vs LoRA side-by-side")
