#!/usr/bin/env python3
"""
Quantitative evaluation: base vs LoRA on a held-out test set.

Metrics:
  - Sentiment accuracy
  - Summary ROUGE-1 / ROUGE-L
  - Format compliance (strict + lenient)

Usage:
  python evaluate.py
  python evaluate.py --test data/movie_reviews_test.jsonl
...
from config import EvalConfig, ModelConfig, QLoRAConfig
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate base vs LoRA on test set")
...
def main():
    ...
    print("=" * 60)
    print("QUANTITATIVE EVAL — Base vs LoRA")
    print("=" * 60)
    ...
    print("\nHow to read this:")
    print("  • Large format delta + small ROUGE delta → mostly style/format learning")
    print("  • Large sentiment + ROUGE delta → genuine task improvement")
    print("  • Strict format = exact 'SENTIMENT: ...\\nSUMMARY: ...' (case-sensitive)")
